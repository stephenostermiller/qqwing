 ...
 */
var randomPossibilityArray = new Array(qqwing.ROW_COL_SEC);

for (var i=0; i<qqwing.ROW_COL_SEC; i++){
...
var onlyPossibilityForCell = function(round){
	for (var position=0; position<qqwing.BOARD_SIZE; position++){
		if (solution[position] == 0){
			var count = 0;
			var lastValue = 0;
			for (var valIndex=0; valIndex<qqwing.ROW_COL_SEC; valIndex++){
				var valPos = getPossibilityIndex(valIndex,position);
...
var onlyValueInRow = function(round){
	for (var row=0; row<qqwing.ROW_COL_SEC; row++){
		for (var valIndex=0; valIndex<qqwing.ROW_COL_SEC; valIndex++){
			var count = 0;
			var lastPosition = 0;
			for (var col=0; col<qqwing.ROW_COL_SEC; col++){
				var position = (row*qqwing.ROW_COL_SEC)+col;
				var valPos = getPossibilityIndex(valIndex,position);
				if (possibilities[valPos] == 0){
					count++;
...
var onlyValueInSection = function(round){
	for (var sec=0; sec<qqwing.ROW_COL_SEC; sec++){
		...
		for (var valIndex=0; valIndex<qqwing.ROW_COL_SEC; valIndex++){
			...
			for (var i=0; i<3; i++){
				for (var j=0; j<3; j++){
					var position = secPos + i + qqwing.ROW_COL_SEC*j;
					var valPos = getPossibilityIndex(valIndex,position);
					if (possibilities[valPos] == 0){
						count++;
						lastPosition = position;
...
var pointingRowReduction = function(round){
	for (var valIndex=0; valIndex<qqwing.ROW_COL_SEC; valIndex++){
		for (var section=0; section<qqwing.ROW_COL_SEC; section++){
			var secStart = sectionToFirstCell(section);
			var inOneRow = true;
			var boxRow = -1;
			for (var j=0; j<3; j++){
				for (var i=0; i<3; i++){
					var secVal=secStart+i+(qqwing.ROW_COL_SEC*j);
...
var hiddenPairInRow = function(round){
	for (var row=0; row<qqwing.ROW_COL_SEC; row++){
		for (var valIndex=0; valIndex<qqwing.ROW_COL_SEC; valIndex++){
			...
			var c2 = -1;
			var valCount = 0;
			for (var column=0; column<qqwing.ROW_COL_SEC; column++){
				var position = rowColumnToCell(row,column);
				var valPos = getPossibilityIndex(valIndex,position);
				if (possibilities[valPos] == 0){
					if (c1 == -1 || c1 == column){
			...
			if (valCount==2){
				for (var valIndex2=valIndex+1; valIndex2<qqwing.ROW_COL_SEC; valIndex2++){
					var c3 = -1;
					var c4 = -1;
					var valCount2 = 0;
					for (var column=0; column<qqwing.ROW_COL_SEC; column++){
					...
					if (valCount2==2 && c1==c3 && c2==c4){
						var doneSomething = false;
						for (var valIndex3=0; valIndex3<qqwing.ROW_COL_SEC; valIndex3++){
...
var hiddenPairInSection = function(round){
	for (var section=0; section<qqwing.ROW_COL_SEC; section++){
		for (var valIndex=0; valIndex<qqwing.ROW_COL_SEC; valIndex++){
			...
			var si2 = -1;
			var valCount = 0;
			for (var secInd=0; secInd<qqwing.ROW_COL_SEC; secInd++){
...
var mark = function(position, round, value){
	...
	var rowStart = cellToRow(position)*qqwing.ROW_COL_SEC;
	for (var col=0; col<qqwing.ROW_COL_SEC; col++){
	...
	for (var i=0; i<qqwing.ROW_COL_SEC; i++){
		var colVal=colStart+(qqwing.ROW_COL_SEC*i);
				...
				if (position != position2){
					...
					if (possibilities2 == 2 && arePossibilitiesSame(position, position2)){
						if (row == cellToRow(position2)){
							var doneSomething = false;
							for (var column2=0; column2<qqwing.ROW_COL_SEC; column2++){
						...
						if (column == cellToColumn(position2)){
							var doneSomething = false;
							for (var row2=0; row2<qqwing.ROW_COL_SEC; row2++){
						...
						if (section == cellToSectionStartCell(position2)){
							...
							for (var i=0; i<3; i++){
								for (var j=0; j<3; j++){
									var position3=secStart+i+(qqwing.ROW_COL_SEC*j);
									if (position3 != position && position3 != position2 && removePossibilitiesInOneFromTwo(position, position3, round)){
										doneSomething = true;
									}
								}
...
var shuffleRandomArrays = function(){
	...
	shuffleArray(randomPossibilityArray, qqwing.ROW_COL_SEC);
};
...
var sudokuToString = function(puz){
	...
	for(var i=0; i<qqwing.BOARD_SIZE; i++){
		...
		} else if (i%qqwing.ROW_COL_SEC==qqwing.ROW_COL_SEC-1){
			if (printStyle == qqwing.PrintStyle.READABLE || printStyle == qqwing.PrintStyle.COMPACT){
				s += "\n";
			}
